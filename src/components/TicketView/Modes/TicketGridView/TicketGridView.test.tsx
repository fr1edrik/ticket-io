...
test('TicketGridView', () => {
  render(<TicketGridView tickets={tickets} />);

  tickets.forEach(({ title, startDate, address, shopUrl }, index) => {
    expect(screen.getByTestId(`ticket_grid_view_title_${index}`)).toHaveTextContent(title);
    expect(screen.getByTestId(`ticket_grid_view_date_${index}`)).toHaveTextContent(moment(startDate).format('dd. DD.MM.YYYY'));
    expect(screen.getByTestId(`ticket_grid_view_time_${index}`)).toHaveTextContent(`${moment(startDate).format('hh:mm')} clock`);
    expect(screen.getByTestId(`ticket_grid_view_address_${index}`)).toHaveTextContent(`${address.streetAddress}, ${address.addressLocality}`);
    expect(screen.getByTestId(`ticket_grid_view_ticket_btn_${index}`)).toHaveAttribute('href', shopUrl);
  });
});
